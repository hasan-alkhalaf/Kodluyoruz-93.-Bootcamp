...
const assistants = data.filter((item) => item.assistant === true); 
 const students = data.filter((item) => item.assistant === false); 
  
 function getAssistants() { 
  assistants.forEach((assistant) => { 
   assistant.members = []; 
   students.filter((student) =>  
	assistant.group === student.group ? assistant.members.push(student) : "" 
   ); 
  }); 
 } 
